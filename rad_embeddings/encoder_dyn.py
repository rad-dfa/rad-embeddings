...
from dfa_gym import DFADynBisimEnv
...
class DynEncoderModule(nn.Module):
    encoder_dim: int = 32
    max_size: int = 10
    n_heads: int = 4
    n_events: int = 10
    ...
    def setup(self):
        ...
        self.tkn_embed = nn.Embed(self.n_events, 32)
        self.combine = nn.Sequential([
            nn.Dense(64),
            nn.relu,
            nn.Dense(self.encoder_dim),
        ])
    ...
    def __call__(
        ...
        g = self.g_embed(h[graph["current_state"]])
        # g = h[graph["current_state"]]
        feat_sym2evt = self.tkn_embed(sym2evt).reshape(sym2evt.shape[0], -1)
        return self.combine(jnp.concatenate([g, feat_sym2evt], axis=-1))
    ...
    def train(
        ...
        env = DFADynBisimEnv(n_events=n_events, sampler=sampler, binary_reward=binary_reward)
        env = LogWrapper(env=env, config=config)

        encoder = cls(max_size=max_size, n_events=n_events)
        ...
        with open(f"{save_dir}/encoder_dyn_params_max_size_{max_size}_n_tokens_{n_tokens}_n_events_{n_events}_seed_{seed}_binary_reward_{binary_reward}_gamma_{gamma}.msgpack", "wb") as f:
...
class ActorCritic(nn.Module):
    ...
    def __call__(self, batch):
        ...
        sym2evt = batch["sym2evt"]
        if sym2evt.ndim == 1:
            sym2evt = jnp.expand_dims(sym2evt, axis=0)
        sym2evt = jnp.concatenate([sym2evt, sym2evt], axis=0)
        ...
        feat = self.encoder(graph, sym2evt)
...
class DynEncoder:
    def __init__(
        ...
        key = jax.random.PRNGKey(seed)
        self.encoder = DynEncoderModule(max_size=max_size, n_events=n_events)
        sampler = RADSampler()
        dfa = sampler.sample(key)
        dfa_graph = dfa.to_graph()
        sym2evt = jnp.zeros((n_tokens,), dtype=jnp.int32)
        self.encoder_ac = ActorCritic(action_dim=n_events, encoder=self.encoder, deterministic=True)
        params = self.encoder_ac.init(key, {"graph_l": dfa_graph, "graph_r": dfa_graph, "sym2evt": sym2evt})
        self.gamma = gamma
        ...
        pattern = re.compile(
            r"encoder_dyn_params_max_size_(\d+)_n_tokens_(\d+)_n_events_(\d+)_seed_(\d+)_binary_reward_(True|False)_gamma_([\d\.eE+-]+)\.msgpack"
        )
        ...
        for fname in os.listdir(storage_dir):
            ...
            if m:
                f_max_size, f_n_tokens, f_n_events, f_seed, f_binary_reward, f_gamma = m.groups()
                f_max_size = int(f_max_size)
                f_n_tokens = int(f_n_tokens)
                f_n_events = int(f_n_events)
                f_seed = int(f_seed)
                f_binary_reward = f_binary_reward == "True"
                f_gamma = float(f_gamma)
                if f_n_tokens == n_tokens and f_n_events == n_events and f_binary_reward == binary_reward and (abs(f_gamma - self.gamma) < 1e-8):
                    candidates.append((f_max_size, f_seed, f_gamma, fname))

        if not candidates:
            raise FileNotFoundError(
                f"No pretrained dynamic alphabet encoder found with n_tokens == {n_tokens}, n_events == {n_events} and binary_reward == {binary_reward}"
            )
        ...
        if chosen is None:
            raise FileNotFoundError(
                f"No pretrained dynamic alphabet encoder found for seed {seed} or fallback seed 42 "
    ...
    def __call__(
        ...
        return jax.lax.stop_gradient(self.encoder.apply(self.encoder_params, graph, sym2evt))
    ...
    def solve(
        ...
        problem = {"graph_l": graph_l, "graph_r": graph_r, "sym2evt": sym2evt}
