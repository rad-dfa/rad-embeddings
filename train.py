...
from rad_embeddings.encoder import SAMPLERS, SAMPLER_ALIASES, SAMPLER_DEFAULT_P, canonical_sampler, parse_p, resolve_p
...
defaults = {name: param.default for name, param in inspect.signature(EncoderModule.train).parameters.items()}
...
parser.add_argument("--sampler", choices=list(SAMPLERS) + list(SAMPLER_ALIASES), default=defaults["sampler"], help="DFA sampler used to generate bisimulation-game pairs: RAD (ReachAvoidDerived), R (Reach) or RA (ReachAvoid)")
parser.add_argument(
    "--p", type=parse_p, default=argparse.SUPPRESS,
    help="Sampler's DFA-size distribution: sizes n are drawn with weight p**n, or uniformly if p is None "
         f"(default: the sampler's own p; {', '.join(f'{name} {cls.p}' for name, cls in SAMPLERS.items())})",
)
parser.add_argument("--binary-reward", action="store_true", default=defaults["binary_reward"], help="Use binary rewards in the bisimulation game")
...
parser.add_argument("--wandb", dest="enable_wandb", action="store_true", default=defaults["enable_wandb"], help="Log to Weights & Biases")
...
run = dict(
    ...
    gamma=args.gamma,
    sampler=canonical_sampler(args.sampler),
    p=resolve_p(args.sampler, getattr(args, "p", SAMPLER_DEFAULT_P)),
)
